...
# V3 ohne TCP-Goal und IK
# ------------------------------------
# edited WHS,, 30.5.2022 #
# -------------------------------------
# Pick and Place
...
joint_goal = group.get_current_joint_values()
joint_goal[0] = np.deg2rad(-74)       # shoulder pan
joint_goal[1] = np.deg2rad(-84)      # shoulder lift
joint_goal[2] = np.deg2rad(98)      # elbow
joint_goal[3] = np.deg2rad(-104)     # wrist1
joint_goal[4] = np.deg2rad(-90)      # wrist2
joint_goal[5] = np.deg2rad(60)       # wrist3
group.go(joint_goal, wait=True)
...
joint_goal = group.get_current_joint_values()
joint_goal[0] = np.deg2rad(-74)       # shoulder pan
joint_goal[1] = np.deg2rad(-81)      # shoulder lift
joint_goal[2] = np.deg2rad(105)      # elbow
joint_goal[3] = np.deg2rad(-114)     # wrist1
joint_goal[4] = np.deg2rad(-90)      # wrist2
joint_goal[5] = np.deg2rad(60)       # wrist3
group.go(joint_goal, wait=True)
print("Reached Grap-Goal", joint_goal)
...
pose_goal.pose.position.z = pose_goal.pose.position.z - 0.02  # 2cm tiefer geht
...
# input(" Go to Home Position => Enter")
joint_goal = group.get_current_joint_values()
joint_goal[0] = np.deg2rad(60)       # shoulder pan
joint_goal[1] = np.deg2rad(-46)      # shoulder lift
joint_goal[2] = np.deg2rad(27)      # elbow
joint_goal[3] = np.deg2rad(-71)     # wrist1
joint_goal[4] = np.deg2rad(-91)     # wrist2
joint_goal[5] = np.deg2rad(90)      # wrist3
group.go(joint_goal, wait=True)
