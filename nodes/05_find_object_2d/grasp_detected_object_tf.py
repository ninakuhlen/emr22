...
# --- 4. get the pose of object_X  from Camera
print("looking for object_9")

if t.frameExists("/world") and t.frameExists("/object_9"):
    (translation, rotation) = t.lookupTransform("world", "object_9", rospy.Time())
    print("detected object position")
    print(t)
    print(translation)
else:
    print("no bject_9 found => exit() ")
    exit()

# --- 5. go to object position
pose_goal = group.get_current_pose()
pose_goal.pose.position  = translation  # from tf-Tree
pose_goal.pose.position.z -= 0.2  # 20cm higherr
group.set_pose_target(pose_goal)
plan = group.plan()
sucess = group.go(wait=True)
print("suc?", sucess)
